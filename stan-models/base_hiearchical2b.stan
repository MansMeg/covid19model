...
parameters {
  ...
  real<lower=0> alpha_mu[P]; // hiearchical alpha mus
  real<lower=0> alpha_sigma[P]; // hiearchical alpha sigmas
  real<lower=0> alpha_global[P]; // global alpha  
...
transformed parameters {
    ...
    // Handle if a parameter is global or hiearchical
    for(p in 1:P){
      if(hiearchical[p] == 1){
        alpha[p,] = alpha_raw[p,]*alpha_sigma[p] + alpha_mu[p];
      } else {
        for(m in 1:M){
          alpha[p,m] = alpha_global[p];
        }
      }
    }
